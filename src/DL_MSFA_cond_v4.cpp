...
#define ZTOL (datum::eps*10.0)
  ...
  // double mnorm_pdf_from_prec(arma::vec eta,arma::vec *mu,arma::mat *prec_chol){
  //   if(eta.n_elem!=(*mu).n_elem || (*mu).n_elem!= (*prec_chol).n_cols )
  //     Rcpp::stop("Error at mnorm_pdf.  eta.n_elem!=mu.n_elem");
  //
  //   arma::vec tmp= (*prec_chol) * (eta- (*mu));
  //   double normal_exp= dot(tmp,tmp)/2;
  //   // cout<<"normal_exp"<<normal_exp<<"\n";
  //
  //   double det_of_prec_chol=sum( log( diagvec ( *prec_chol) ));
  //   // cout<<"det_of_prec_chol="<<det_of_prec_chol<<"\n";
  //   // double s= -log (2*datum::pi)*(*prec_chol).n_cols/2 + det_of_prec_chol- normal_exp;
  //   double s=  det_of_prec_chol- normal_exp;
  //
  //   return (exp(s));
  // }
  ...
  void update_loading(const arma::mat &eta, arma::mat &Lambda,arma::mat &T_phi,double &T_sum,  const arma::mat &Y,double &tau, const arma::vec &ps,const double a){
    ...
    cout<<"phi updated T_sum="<<T_sum<<endl;
    if(T_phi.has_nan()){
      T_phi.print("T_phi :");
...
//' @examples
  ...
  // [[Rcpp::export]]
  Rcpp::List cov_est_HMC(double ms, double ss, double a,
                         int nrun, int burn, int thin, unsigned nleapfrog,arma::vec del_range,
                         arma::mat phimat, arma::mat Y,unsigned leapmin=5,unsigned leapmax=15){
    ...
    for(int i=0; i<nrun+burn; ++i){
      ...
      /*arma::mat del_v_theta= del_v(phimat_new, ps, Plam, S, del_inv_S, n);

       unsigned nstep=GSL_MAX_DBL(1,R::rpois(nleapfrog));
       // double h=nstep*pow(d,-.25);
       delta= R::runif(.004,.01 );
       leapfrog(nstep, delta,phimat_new, p_lam, ps, S, n, del_inv_S,Plam, del_v_theta);

       H_prior_old=  accu(square(phimat) %Plam)/2;
       H_prior_new=  accu(square(phimat_new) %Plam)/2;

       //get H_ll_new
       // covmat=diagmat(ps)+tcrossprod(phimat_new);
       H_ll_new = (n*fast_fact_det(phimat_new,ps)+ trace(del_inv_S ) )/2 ;

       double H_new= H_ll_new+ H_prior_new+ accu(square(p_lam))/2, H_old=H_ll_old+H_prior_old+kin_energy;*/
      ...
      if(remainder==0 ){
        ...
        ps_mat.row(quotient-1)=(exp(ps)).t();
  ...
  // [[Rcpp::export]]
  Rcpp::List cov_est(double as, double bs, double a,
                     int nrun, int burn, int thin,
  ...
  Rcpp::List SUFA_HMC(const unsigned nrun,const unsigned thin, unsigned nleapfrog,const arma::vec del_range,
    ...
    for(unsigned i=0; i<nrun; ++i){
      ...
      // arma::vec Hvec(nstudy);
      double Hvec=0;
      omp_set_num_threads(1);
#pragma omp parallel for num_threads(nthreads) reduction(+:Hvec)
      for(unsigned j=0;j<nstudy;++j)
        Hvec+= ( ns(j) *fast_fact_det(Phi_sq_As[j],exp(params_new.ps))+ trace(del_inv_S.slice(j) )
      ...
      /*cout<<"H_ll_new= "<<H_ll_new<<" H_ll_old= "<<H_ll_old<<endl;
       cout<<"H_new= "<<H_new<<" H_old= "<<H_old<<endl;*/
